...
use jpeg_encoder::{Encoder, EncodingError, ColorType};
...
pub fn embed_in_image(
    ...
    let bit_depth = config.parameters.get("bit_depth")
...
fn embed_bytes(buffer: &mut ImageBuffer<Rgba<u8>, Vec<u8>>, start_bit: usize, data: &[u8], bit_depth: u8) -> Result<()> {
    ...
    for &byte in data {
        for bit in 0..8 {
            ...
            let bit_value = ((byte >> (7 - bit)) & ((1 << bit_depth) - 1)) as u8;
...
pub fn save_image_with_embedded_data(
    image: &DynamicImage,
    output_path: &Path,
) -> Result<()> {
...
pub fn embed_data(config: EmbedConfig) -> Result<()> {
    ...
    let processed_data = if let Some(crypto_config) = config.encryption {
        match crypto_config.algorithm {
            Algorithm::Aes256 => {
                crate::encryption::encrypt(&config.data, &crypto_config)?
            }
            Algorithm::ChaCha20 => {
                crate::encryption::encrypt(&config.data, &crypto_config)?
            }
...
pub fn embed_in_jpg(config: EmbedConfig) -> Result<()> {
    ...
    let jpeg_info = match decoder.info() {
        Some(info) => info,
        None => return Err(Error::InvalidInput("Failed to get JPEG information".to_string())),
    };
    ...
    if pixel_format != PixelFormat::RGB24 && pixel_format != PixelFormat::CMYK32 {
        return Err(Error::InvalidInput(
            format!("Unsupported pixel format: {:?}, only RGB24 and CMYK32 are supported", pixel_format)
        ));
    }
    ...
    println!("Debug: Image dimensions: {}x{}, pixel format: {:?}", width, height, pixel_format);
    println!("Debug: Maximum blocks: {}x{}, capacity: {} bytes", max_blocks_x, max_blocks_y, capacity_bytes);
    ...
    println!("Debug: Embedding {} bits ({} bytes with error correction)", bits.len(), protected_data.len());
                            ...
                            for x in bx * 8..(bx + 1) * 8 {
                                if y < height && x < width {
                                    ...
                                    if pixel_pos < modified_pixels.len() {
                                        ...
                                        let new_value = modified_pixels[pixel_pos] as i16 + adjustment;
    ...
    let mut encoder = Encoder::new(BufWriter::new(output_file), 95); // Higher quality to preserve our data
    encoder.encode(&modified_pixels, width as u16, height as u16, color_type)
...
mod tests {
    ...
    fn test_embed_data() -> Result<()> {
        ...
        doc.save(&input_path).map_err(|e| Error::Io(e.to_string()))?;
    ...
    fn test_jpg_steganography() {
        ...
        println!("Test: Created {} bits from {} bytes of data", data_with_length.len() * 8, data_with_length.len());
        ...
        for by in 0..8 { // 8 blocks vertically
            for bx in 0..8 { // 8 blocks horizontally
                if bit_index < bits.len() {
                    // Calculate the average blue value for this 8x8 block
                    let mut blue_sum = 0;
                    let mut pixel_count = 0;
