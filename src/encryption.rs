...
fn decrypt_aes256(encrypted: &[u8], key: &[u8]) -> Result<Vec<u8>> {
    ...
    // Remove padding
    let padding_len = *decrypted.last().ok_or_else(|| Error::Encryption("Empty decrypted data".into()))? as usize;
...
fn encrypt_rsa(data: &[u8], key: &[u8], output: &mut Vec<u8>) -> Result<()> {
    ...
    let enc_key = public_key.encrypt(&mut rng, PaddingScheme::new_pkcs1v15_encrypt(), &aes_key)
...
fn decrypt_rsa(encrypted: &[u8], key: &[u8]) -> Result<Vec<u8>> {
    if encrypted.len() < 4 {
        return Err(Error::Encryption("Invalid encrypted data".into()));
    }
    ...
    let aes_key = private_key.decrypt(PaddingScheme::new_pkcs1v15_encrypt(), enc_key)
