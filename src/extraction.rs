...
pub fn extract_from_pdf(config: ExtractConfig) -> Result<Vec<u8>> {
    ...
    if let Some(crypto_config) = config.encryption {
        match crypto_config.algorithm {
            Algorithm::Aes256 => {
                crate::encryption::decrypt(&raw_data, &crypto_config)
            }
            Algorithm::ChaCha20 => {
                crate::encryption::decrypt(&raw_data, &crypto_config)
            }
...
fn extract_from_image(image: &DynamicImage, bit_depth: u8) -> Result<Vec<u8>> {
    ...
    if data_len as u32 > max_bytes {
        return Err(Error::InvalidData(format!("Data length ({}) exceeds image capacity ({})", data_len, max_bytes)));
    }
...
fn extract_bytes(buffer: &ImageBuffer<Rgba<u8>, Vec<u8>>, start_bit: usize, data: &mut [u8], bit_depth: u8) -> Result<()> {
...
pub fn extract_from_jpg(config: ExtractConfig) -> Result<Vec<u8>> {
    ...
    println!("DEBUG: Max blocks: {}x{}, capacity: {} bytes", max_blocks_x, max_blocks_y, capacity_bytes);
    ...
    if raw_bytes.len() < 4 {
        return Err(Error::InvalidData("Not enough data in the image".to_string()));
    }
    ...
    if data_length + 4 > raw_bytes.len() || data_length > capacity_bytes {
        return Err(Error::InvalidData(format!(
            "Invalid data length: {}. Max capacity: {}", data_length, capacity_bytes - 4
        )));
    }
    ...
    println!("DEBUG: Extracted {} bytes of error-corrected data", error_corrected_data.len());
...
pub fn extract_from_wav(_config: ExtractConfig) -> Result<Vec<u8>> {
    ...
    Err(Error::NotImplemented("WAV extraction not yet implemented".into()))
}
...
pub fn extract_from_mp3(_config: ExtractConfig) -> Result<Vec<u8>> {
    ...
    Err(Error::NotImplemented("MP3 extraction not yet implemented".into()))
}
...
pub fn extract_from_mp4(_config: ExtractConfig) -> Result<Vec<u8>> {
    ...
    Err(Error::NotImplemented("MP4 extraction not yet implemented".into()))
}
