...
impl PdfHandler {
    ...
    pub fn embed_data(&mut self, data: &[u8]) -> Result<()> {
        ...
        stream.dict.set("Type", Object::Name("EmbeddedFile".as_bytes().to_vec()));
    ...
    pub fn extract_data(&self) -> Result<Vec<u8>> {
        ...
        let metadata = catalog.get(b"Metadata")
        ...
        if let Object::Reference(reference) = metadata {
            ...
            match self.doc.get_object(*reference)
...
mod tests {
    ...
    fn test_pdf_embed_extract() {
        ...
        let output_path = temp_file.path().with_extension("output.pdf").to_str().unwrap().to_string();
